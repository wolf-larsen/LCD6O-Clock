...
//Serial.printf("(%ld) al1=%s bloff=%s \n",millis(),(const char *)config["alarm"]["Alarm1"],(const char *)config["display"]["backlightOffTime"]);
        //--- Test horaire declenchement alarme
        ...
        //--- Test horaire allumage/extinction backlight
#ifdef DEBUGTIME
Serial.printf("(%ld) pass 1 \n",millis());
#endif
...
#ifdef DEBUGTIME
Serial.printf("(%ld) sleeping=%d humanpresence=%d \n",millis(),sleeping,humanPresence);
#endif
        if (sleeping || ! humanPresence) {
