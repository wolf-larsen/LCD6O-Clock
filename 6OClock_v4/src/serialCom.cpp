...
void afterCommChecked() {
  ...
  orderSend("SETBLINK", (bool)config["display"]["blinkSecondsLeds"] ? "ON" : "OFF", false);
...
//      }
      ...
      // ---- getblinkmode ---------------
      else if (strcmp(messagesRecords[idx].order, "GETBLINK")==0) {
        responseSend(messagesRecords[idx].id, (bool)config["display"]["blinkSecondsLeds"] ? "ON" : "OFF");
        freeMessage(idx);
      }
      // ---- setdisplaymode ---------------
      ...
      // ---- change numbers font ---------------
        ...
        int displayFaceIndex = (long)config["display"]["faceNumber"];
        displayFaceIndex++;
...
//Serial.printf(" (rqt=%s|%s)\n",messagesRecords[orderFound].order,messagesRecords[orderFound].text);
        ...
        if (strcmp(messagesRecords[orderFound].order, "COMMCHECK")==0) {
          CommChecked = true;
          scheduler.addOneShotEvent(afterCommChecked, 1);
        }
...
#ifdef DEBUGPROTO
//if (strcmp(command, "TOP")!=0) {Serial.printf("(%ld) ordersend ordre=%s/%s \n",millis(),command,text);}
#endif
  if (command != NULL) {
    int16_t idxFree = -1;
    for (uint16_t idx=0; idx<MAX_MESSAGES; idx++) {
...
#ifdef DEBUGPROTO
if (strcmp(command, "TOP")!=0) {Serial.printf("(%ld) ordre %ld:%s|%s envoye\n",millis(),ordersCount, command, text);}
#endif
